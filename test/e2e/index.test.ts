...
test('home page opens Slack install flow', async ({ page }) => {
  await page.route('**/api/slack/install', async (route) => {
    await route.fulfill({
  ...
  await expect(addToSlack).toHaveAttribute('href', '/api/slack/install')

  await addToSlack.click()

  await expect(page).toHaveURL('/api/slack/install')
