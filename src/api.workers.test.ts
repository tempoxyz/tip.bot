...
describe('/api/chat/telegram', () => {
  test('group mention tip resolves known Telegram username', async () => {
    ...
    const handleTipRequest = vi.spyOn(Tip, 'handleTipRequest').mockResolvedValue({
      ...
      memo: 'coffee',
      ok: true,
      recipientProviderUserId: '200',
      senderProviderUserId: '100',
      status: 'sent',
      tokenCurrency: 'USD',
      tokenSymbol: 'PathUSD',
      transactionHash: `0x${'1'.repeat(64)}`,
    })
    const telegramPosts: unknown[] = []
    server.use(
      ...
      mswHttp.post('https://api.telegram.org/bot000000:test/getWebhookInfo', () =>
        HttpResponse.json({
          ...
          result: {
            ...
            url: 'https://tip.bot/api/chat/telegram',
          },
        }),
      ),
      mswHttp.post('https://api.telegram.org/bot000000:test/sendMessage', async ({ request }) => {
        telegramPosts.push(await request.json())
        return HttpResponse.json({
          ok: true,
          result: {
    ...
    const response = await api.fetch(
      new Request('https://tip.bot/api/chat/telegram', {
        body: JSON.stringify({
          message: {
        ...
        headers: {
          ...
          'x-telegram-bot-api-secret-token': env.TELEGRAM_WEBHOOK_SECRET_TOKEN,
        },
        method: 'POST',
      }),
      env,
    ...
    expect(handleTipRequest).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        memo: 'coffee',
        provider: 'telegram',
        providerChannelId: 'telegram:-100123',
        providerId: '-100123',
