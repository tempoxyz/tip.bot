...
function writeGeneratedFile(filePath: string, output: string) {
  ...
  execFileSync('pnpm', ['run', 'format', '--', outputPath], {
    cwd: path.resolve(import.meta.dirname, '..'),
    stdio: 'inherit',
  })
...
writeGeneratedFile('src/lib/db.gen.ts', output)
writeGeneratedFile('src/lib/db.schemas.gen.ts', schemaOutput)
